...
impl<T: Num + Copy> SymmetricMatrix<T> {
    ...
    pub fn new_from_func<F>(n: usize, func: F) -> Self
    where
        F: Fn(usize, usize) -> T,
    {
        let mut uninit_matrix = SymmetricMatrixUninit::<T>::new(n * (n + 1) / 2);
        for i in 0..n {
            for j in 0..n {
                unsafe {
                    uninit_matrix.set_uncheked(i, j, func(i, j));
                }
            }
        }
...
/*     pub fn save_to_npy(&self,file:&mut std::fs::File){
        ...
        default_dtype().
        shape(&shape).writer(file).begin_nd().unwrap();
        for i in 0..self.dimension{
            for j in 0..self.dimension{
                unsafe {
                    writer.push(&self.get_unchecked(i, j)).unwrap();
                }
            }
        }
    } */
...
/* struct SymmetricMatrixIterator<'a,T>{
    matrix: &'a SymmetricMatrix<T>,
    i:usize,
    j:usize,
}

impl<T> Iterator for SymmetricMatrixIterator<'_,T>{
    type Item=T;
    fn next(&mut self) -> Option<Self::Item>{
        let n=self.matrix.dimension;
        if self.i==n-1 && self.j==n-1 {
            return None;
        }
        Some(self.matrix.;
    }
...
mod tests {
    ...
    fn test_new_from_func() {
        let n = 5;
        let matrix = SymmetricMatrix::new_from_func(n, |i, j| i * j);
        for i in 0..n {
            for j in 0..n {
                assert_eq!(matrix.get(i, j), i * j);
            }
        }
    }
