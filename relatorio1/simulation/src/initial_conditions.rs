...
pub fn create_two_capacitors<T:Float>(n:usize,plate_separation:T,plate_length:T,plate_potential:T)->EletricPotential<T>
{
    let mut potential=Array2::<T>::from_elem((n,n),T::zero());
    let mut fixed_points=Array2::<bool>::from_elem((n,n),false);
    for i in 0..n
    ...
    let plate_separation=(plate_separation*T::from(n).unwrap()).round().to_usize().unwrap();
    let plate_length=(plate_length*T::from(n).unwrap()).round().to_usize().unwrap();
    ...
    return EletricPotential {potential_array:potential,fixed_points};
}